def get_data(dir_endo, dir_cancer):
    """
    Returns the merged data of endometriosis and cancer
    Inputs: folder direcory of endometriosis and cancer (in this order)
    Output: merged Anndata object of both datasets
    """
    # Import libraries
    import tarfile
    import scanpy as sc
    ...
    data_dir = dir_endo
    with tarfile.open(f"{dir_endo}/GSE214411_RAW.tar", "r") as tar:
        tar.extractall(path=dir_endo)
    ...
    adata_endo = adata_endo[adata_endo.obs.n_genes_by_counts < 2500, :] # to remove doublets
    adata_endo = adata_endo[adata_endo.obs.pct_counts_mt < 20, :].copy() # to remove cell with high mitochondrial activity
    ## Ovarian Cacner Data
    # For loop to create an adata for the cancer dataset
    # Extract files to a directory
    data_dir = dir_cancer
    with tarfile.open(f"{dir_cancer}/GSE184880_RAW.tar", "r") as tar:
        tar.extractall(path=dir_cancer)
    # List all prefixes (one per sample)
    prefixes = [
    "GSM5599220_Norm1",
    "GSM5599221_Norm2",
